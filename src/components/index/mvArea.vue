<template>
	<div class="kv overflow-hidden">
		<hexagon />
		<div class="kv-item" :class="{ scroll: kvScroll, animate: active == item - 1 || preactive == item - 1 }" v-for="item in total" :style="img(item)"> </div>
		<div class="">
			<div class="transition-all duration-300 absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2" :style="{ opacity: !kvScroll ? '1' : '0' }">
				<h1 class="font-bold text-[50px] md:text-[100px] text-white capitalize tracking-wider">Ned Lin</h1>
			</div>
			...
			<div class="transition-all duration-300 absolute bottom-[16%] md:bottom-[10%] left-1/2 -translate-x-1/2 text-center" :style="{ opacity: kvScroll ? '1' : '0' }">
				<h1 class="font-extrabold text-[30px] md:text-[60px] text-black whitespace-nowrap tracking-wider">Case</h1>
			</div>
          ...
          <h1 class="font-bold text-[20px] md:text-[36px] text-[#07b53b] whitespace-nowrap">Experience is the best teacher.</h1>
        </div> -->
		</div>
	</div>
</template>
...
<script lang="ts" setup>
	...
	const ai01 = ref(`/sp-LoginPage/assets/img/ai01.png`);
	...
	const img = (n: number) => {
		return {
			backgroundImage: `url(/sp-LoginPage/assets/img/ai0${n}.png)`,
...
<style lang="scss" scope>
	...
	.scroll-after {
		...
		&::after {
			...
			left: 14px;
			bottom: 0px;
			animation: scrollMove 0.9s ease-in-out infinite;
		}
	}
